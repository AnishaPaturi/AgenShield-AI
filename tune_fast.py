...
def generate_paper_with_paragraphs(num_sections):
    doc = docx.Document()
    ...
    # Dynamic expansion paragraphs controlled by num_sections
...
def run():
    ...
    for n in range(10, 35):
        docx_path = generate_paper_with_paragraphs(n)
        doc = word.Documents.Open(docx_path)
        pages = doc.ComputeStatistics(1) # wdStatisticPages
        print(f"Sub-sections={n} => Page Count: {pages}", flush=True)
        doc.Close(False)
        if pages == 16:
            print(f"SUCCESS! Found exact 16 pages at sub-sections count = {n}", flush=True)
            best_n = n
            break
        elif pages > 16:
            best_n = n - 1
            print(f"Passed 16 pages. Generating final document with n={best_n}", flush=True)
            generate_paper_with_paragraphs(best_n)
            doc2 = word.Documents.Open(docx_path)
            final_p = doc2.ComputeStatistics(1)
            print(f"Final Page Count verified: {final_p} pages", flush=True)
            doc2.Close(False)
            break
