...
class ValidatorAgent:
    """Specialized Code & Sandbox Validator Agent executing static lint verification, LocalStack dry-run, and rollback."""

    def __init__(
        self,
        custom_linters: dict[IaCType | str, list[BaseLinter]] | None = None,
        sandbox: LocalStackSandbox | None = None,
    ) -> None:
        self.custom_linters = custom_linters or {}
        self.sandbox = sandbox or LocalStackSandbox()
    ...
    def validate_patch(
        ...
        """Validate a single PatchDiff against static linters and LocalStack runtime sandbox with rollback.
        ...
        all_linters_passed = all(r.passed for r in results)

        # 3. If static linters pass, execute Task 4.3 LocalStack runtime sandbox dry-run
        if all_linters_passed:
            ...
            sandbox_res = self.sandbox.validate_runtime(
            ...
            if sandbox_res.passed:
                patch.remediation_status = RemediationStatus.SANDBOX_PASSED
                logger.info("Patch %s PASSED LocalStack sandbox dry-run.", patch.patch_id)
                return patch

            logger.warning(
                "Patch %s FAILED LocalStack sandbox dry-run: %s",
                patch.patch_id,
                sandbox_res.error,
            )
        else:
        ...
        # 4. Handle Lint or Sandbox Failure: Enforce Automated Rollback & Re-Remediation Loop
        failed_checks = [r for r in results if not r.passed]
        lint_errors = [f"{r.check_name}: {r.error or r.output}" for r in failed_checks]
        logger.warning(
            "Patch %s FAILED validation checks: %s. Executing automated rollback.",
            patch.patch_id,
            lint_errors,
        )
