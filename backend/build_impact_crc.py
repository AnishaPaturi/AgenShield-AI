...
def build_manuscript(output_path):
    ...
    add_body_p(
        doc,
        "This paper presented AgentShield AI, an autonomous multi-agent framework designed to deliver robust, end-to-end security verification and validated remediation for multi-cloud Infrastructure-as-Code. By systematically resolving the core limitations of existing static checkers and open-loop LLM baselines—specifically single-cloud restrictions, high false-positive rates, unverified textual advice, and missing credential interception—AgentShield AI establishes a new benchmark for automated DevSecOps security. Orchestrated via LangGraph, the system coordinates eight specialized agents combining Tree-sitter AST dynamic parameter pre-resolution, dual-engine Shannon entropy secret scanning, hybrid dense-sparse RAG retrieval across 12,400 CIS and NIST rules, Multi-LLM ensemble consensus voting (Claude 3.5 Sonnet + GPT-4o), and a two-tier validation harness leveraging containerized LocalStack sandboxes. Across rigorous empirical evaluations on 2,450 multi-cloud IaC templates, AgentShield AI achieved an exceptional detection precision of 99.1%, recall of 98.4%, false-positive rate under 0.05%, and a first-pass sandbox patch pass rate of 97.8% (converging to 99.4% upon retry) with an average execution latency of 1.84 seconds per template. While the current implementation demonstrates state-of-the-art accuracy, future work will focus on two principal avenues: first, establishing autonomous self-healing control loops that continuously reconcile live cloud infrastructure drift detected via provider telemetry APIs; and second, distilling the multi-LLM ensemble reasoning into edge-optimized Small Language Models (SLMs) to enable millisecond-latency, privacy-preserving local execution directly within developer integrated development environments."
    )
