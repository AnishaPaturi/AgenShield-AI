# generate_full_12page_text.py
"""
Generates the comprehensive 8,500-word dataset in paper_data.py
ensuring standard IEEE 8.0pt typography hits exactly 12.0 pages.
"""
...
Target: Exactly 12.0 pages in standard IEEE two-column format at 8.0 pt typography.
"""
...
SECTIONS = {
    "I. INTRODUCTION": [
        ...
        (
            "<b>A. Multi-Cloud IaC Vulnerability Taxonomy</b><br/>"
            "Cloud infrastructure vulnerabilities manifest across four primary architectural domains within modern enterprise environments:"
            "<br/>1) <i>Storage & Object Persistence:</i> Cloud storage volumes (Amazon S3, Azure Blob, Google Cloud Storage) frequently omit default "
            "server-side encryption (SSE-KMS), lack access logging, and fail to enforce public access blocks, resulting in unauthorized data exposure."
            "<br/>2) <i>Network Perimeter & Ingress Security:</i> Security groups and network access control lists (NACLs) are frequently defined with open "
            "CIDR ingress blocks (0.0.0.0/0), exposing sensitive internal ports (e.g., PostgreSQL port 5432, Redis port 6379) directly to the Internet."
            "<br/>3) <i>Identity and Access Management (IAM):</i> Over-privileged role definitions granting wildcard administrative actions (Action: *, "
            "Resource: *) allow attackers who compromise a single compute workload to escalate privileges and take over the entire cloud tenancy."
            "<br/>4) <i>Container Workloads & Kubernetes Pods:</i> Container manifests frequently request root execution privileges (runAsUser: 0), share the "
            "host network namespace (hostNetwork: true), and omit memory/CPU resource quotas, creating pathways for container escape and host takeover."
        ),
        (
            "<b>B. The Fragility of Existing IaC Security Methodologies</b><br/>"
...
''')
    print("paper_data.py successfully created.")
